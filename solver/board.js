...
class Board {
	constructor(rows = 8, cols = 6) {
		this.rows = rows;
		this.cols = cols;
		this.fields = Array.from({ length: rows }, () => Array.from({ length: cols }, () => new Field()));
	...
	click(row, col) {
		...
		switch (mod) {
			...
			case "x":
				...
				return true;
			default:
				console.log("Unknown modifier", mod);
				return false;
		}
	}
	...
	toData() {
		return {
			...
			moveSequence: this.moveSequence.moves.map((m) => ({ row: m.row, col: m.col })),
			hasBombs: this.hasBombs,
		};
	}
	...
	static fromData(data) {
		...
		for (let r = 0; r < data.rows; r++) {
			for (let c = 0; c < data.cols; c++) {
				...
				f.onlyReachableFrom = new Position(fData.onlyReachableFrom.row, fData.onlyReachableFrom.col);
			}
		}
		b.moveSequence.moves = data.moveSequence.map((m) => new Position(m.row, m.col));
		b.moveSequence.n = data.moveSequence.length;
